...
export default function PostMedia({ postData, visibility, observing }) {
  ...
  useEffect(() => {
    ...
    if (videoPlayer.current) videoPlayer.current.pause();
  }, []);
  ...
  const handleTimeUpdate = () => {
    setVideoProgress(
      (100 / videoPlayer.current.duration) * videoPlayer.current.currentTime
    );
  };
  ...
  return (
    <>
      {postData.srcType === "video" && (
        <>
          <div
            className={`${buffering} ${styles.buffering} ${styles.controls} absolute`}
          >
            <Loading whiteSpinner={true} />
          </div>
          <div
            className={`${styles.postMediaContainer} ${visibility} display-flex-column align-end justify-end absolute width-100 height-100`}
          >
            <div
              onClick={handleRestart}
              className={`${styles.controls} padding-10`}
            >
              <Replay height={40} />
            </div>
            <div
              onClick={handleVolume}
              className={`${!mute ? "display-block" : "display-none"} ${
                styles.controls
              } padding-10`}
            >
              <VolumeOn height={40} />
            ...
            <div
              onClick={handlePlay}
              className={`${!playing ? "display-block" : "display-none"} ${
                styles.controls
              } padding-10`}
            >
              <Play height={40} />
            ...
            <div
              ...
              className={`${styles.progressBar} gradient-success absolute`}
            ></div>
          </div>
          <video
            ref={videoPlayer}
